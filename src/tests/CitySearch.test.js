...
describe("<CitySearch /> component", () => {
  ...
  let locations;
  beforeAll(() => {
    locations = extractLocations(mockData.items);
  ...
  test("render list of suggestions correctly", () => {
    ...
    for (let i = 0; i < suggestions.length; i += 1) {
      const listItemText = CitySearchWrapper.find(".suggestions li")
        .at(i)
        .text();
      expect(listItemText).toBe(suggestions[i] || "");
    }
    expect(CitySearchWrapper.find(".suggestions li").last().text()).toBe(
      "See All Cities"
    );
  });
